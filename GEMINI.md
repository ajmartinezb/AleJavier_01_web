# GEMINI.md — instrucciones del proyecto para Gemini

> Este archivo es el equivalente de `CLAUDE.md` para **Gemini CLI**, que lo lee automáticamente
> al abrir una sesión en esta carpeta. Si trabajas en la **web de Gemini** (gemini.google.com),
> pega o sube este archivo al comenzar, o úsalo como instrucciones de un Gem.
> La guía completa para el usuario está en `guia-gemini/GUIA.md`.

## Cómo trabajar en este proyecto (resumen para Gemini)

1. **Antes de escribir**, lee la pauta del libro (`biologia/pauta.md`, y además `fisica/pauta.md` si es Física),
...
4. Después de escribir, genera y revisa:
   ...
   python guia-gemini/scripts/reparto_claves.py <libro>/capitulos/<capNN-slug>/preguntas.json
   ```
   y corrige hasta que `"problemas": []` quede vacío y los repartos estén dentro de rango.
5. Al cerrar, actualiza `<libro>/progreso.md` y `<libro>/temario.md`.
6. Escribe siempre en **español de Chile**, con redacción propia (el material es público: no copies textos de libros ni de sitios web)
   y registra cada fuente usada en `99-fuentes.md` del capítulo. No inventes datos: si no puedes verificar una cifra, dilo.
